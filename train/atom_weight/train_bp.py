...
atm_weight = [42, 1, 1, 1, 21, 3, 6, 5, 1, 1, 1, 1, 1, 2, 4, 1, 1, 1]
mol_num_tr = np.arange(21)
mol_num_val = [21, 22, 23, 26, 27, 28, 29, 30]
...
def loss_postscf_atm():
    ...
    for i in range(N_atm_tr):
        ...
        E_loss += torch.abs(e_loss * atm_weight[i])
        D_loss += d_loss * atm_weight[i]
        tot_loss = torch.abs(e_loss) + const2 * d_loss
        Loss += tot_loss * atm_weight[i]
        E_del_sum += E_delta
        D_del_sum += D_delta
...
pp = torch.load(file_x_init_name)
s_nn.load_state_dict(pp)
print(s_nn.state_dict())
for epoch_scf in range(n_scf):
    print("New SCF epoch:", epoch_scf)
    loss_scf_cal()
    for epoch_postscf in range(n_postscf):
        print("New postscf epoch:", epoch_scf, "%", epoch_postscf)
        opt.zero_grad()
        loss_tr = loss_postscf_cal()
        loss_tr.backward()
        opt.step()
    if epoch_scf == n_scf - 1:
        print("New SCF epoch:", epoch_scf + 1)
        loss_scf_cal()
